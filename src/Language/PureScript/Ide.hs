...
import "monad-logger" Control.Monad.Logger (MonadLogger, logInfoN, logWarnN, LoggingT)
...
import Language.PureScript.Ide.Command (Command(..), ImportCommand(..), ListType(..), RebuildFile (..), ConfigParams (ConfigParams))
...
import Language.PureScript.Ide.Rebuild (rebuildFileAsync, rebuildFileSync, rebuildManyAsync, rebuildAll, rebuildOneAsync, dayZero)
...
import Language.PureScript.Ide.Logging (runLoggerWithFile)
import Language.PureScript.Ide.State qualified as S
...
handleCommand c = case c of
  ...
  RebuildAll targets -> do
    runWithFileLogger (rebuildAll targets)
  RebuildMany files targets ->
    runWithFileLogger $ rebuildManyAsync ((\RebuildFile {..} -> (rfFile, rfText)) <$> files) targets
  RebuildOne file targets ->
    runWithFileLogger $ rebuildOneAsync ((\RebuildFile {..} -> (rfFile, rfText)) file) targets
  UpdateConfig params ->
      updateConfig params
  Cwd ->
...
runWithFileLogger ::
  (MonadReader IdeEnvironment m, MonadError IdeError m) =>
  ReaderT   IdeEnvironment (LoggingT (ExceptT IdeError m)) a -> m a
runWithFileLogger action = do
    env <- ask
    let ll = confLogLevel (ideConfiguration env)
    let logHandle = ideLogHandle env
    res <- runExceptT (runLoggerWithFile ll logHandle (runReaderT action env))
    case res of
...
outputDirectory = do
  outputPath <- confOutputPath <$> S.readIdeConfiguration
...
loadModules moduleNames = do
  ...
  traverse_ (insertExterns . (dayZero,)) efiles
...
loadModules_ = \case
  [] -> do
    _ <- rebuildAll (Set.singleton P.JS)
    pure (TextResult "Loaded (initRebuild)")
  modules ->
    loadModules modules


updateConfig :: (Ide m, MonadError IdeError m, MonadLogger m) => ConfigParams -> m Success
updateConfig (ConfigParams {..}) = do
    -- IdeConfiguration
    -- { confLogLevel = LogAll
