...
diffExterns newExts oldExts depsDiffs =
  -- ExternsDiff (P.efModuleName newExt) reExports [] [] decls
  (trace $ "\n===== diffExterns modName =====: " <> show modName)
    (trace $ "\ndepsDiffs: " <> show depsDiffs)
    (trace $ "\affectedReExported:\n" <> show affectedReExported)
    (trace $ "\nchangedRefs:\n" <> show changedRefs)
    (trace $ "\ndiffsMapWithLocal:\n" <> show diffsMapWithLocal)
    (trace $ "\nunchangedRefs:\n" <> show unchangedRefs)
    (trace $ "\naffectedLocalRefs:\n" <> show affectedLocalRefs)
    (trace $ "\nallAffectedLocalRefs:\n" <> show allAffectedLocalRefs)
    ExternsDiff
    modName
...
checkDiffs (P.Module _ _ mn' decls exports) diffs
  ...
  where
    ...
    searches =
      (trace $ "\ncheckDiffs:" <> show mn')
        (trace $ "\ncheckDiffs:" <> show diffs)
        (trace $ "\ncheckDiffs: mbSearch:\n" <> show mbSearch)
        (fromMaybe S.empty mbSearch)
